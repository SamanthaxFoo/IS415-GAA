...
```{r}
#| eval: false
# Prepare GIFs of LISA maps by fiscal year
...
if (length(temporal_cat_maps) > 0) {
  tmap_animation(temporal_cat_maps, filename = "images/Overall_LISA_by_year.gif", delay = 150, width = 1090, height = 674)
} else {
...
if (length(temporal_sig_maps) > 0) {
  tmap_animation(temporal_sig_maps, filename = "images/Significant_LISA_by_year.gif", delay = 150, width = 1090, height = 674)
} else {
  stop("No significant LISA maps found for animation.")
}
```

![](images/Overall_LISA_by_year.gif){fig-align="center" width="1093"}
...
![](images/Significant_LISA_by_year.gif){fig-align="center" width="1093"}
...
Lastly, we will perform EHSA analysis by using [`emerging_hotspot_analysis()`](https://sfdep.josiahparry.com/reference/emerging_hotspot_analysis.html) of the sfdep package.
...
9.  Skochko, S. (2024, September 5). Asthma Hot Spots in New York Before and During the COVID-19 Pandemic. CDC. Retrieved October 8, 2024, from <https://www.cdc.gov/pcd/issues/2024/24_0059.htm>
10. vsp.pnnl.gov. (n.d.). Mann-Kendall Test For Monotonic Trend. Design Trend Mann-Kendall. Retrieved October 6, 2024, from <https://vsp.pnnl.gov/help/vsample/design_trend_mann_kendall.htm> 
